...
class PreActionSafetyLayer:
    ...
    def analyze_action(self, user_input, available_context=None, action_metadata=None):
        ...
        requires_confirmation = self._requires_confirmation(metadata, risks)
    ...
    def _estimate_confidence(self, user_input, context, metadata, risks):
        score = 0.9
        if len((user_input or "").strip()) < 12:
            score -= 0.15
        if not context:
            score -= 0.1
        if not metadata.get("action_name"):
            score -= 0.1
        score -= min(0.45, len(risks) * 0.1)
    ...
    def _secure_practices(self, context, metadata, risks):
        ...
        if metadata.get("action_name") in self.SENSITIVE_ACTIONS or risks:
            practices.append("Separate development and creator environments to reduce blast radius.")
        return practices

    def _requires_confirmation(self, metadata, risks):
        return metadata.get("action_name") in self.SENSITIVE_ACTIONS or bool(risks)
